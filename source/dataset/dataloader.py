...
def init_stratified_dataloader(cfg: DictConfig,
    ...
    for train_index, test_valid_index in split.split(final_pearson, stratified):
        final_pearson_train, final_timedelay_train, final_its_train, labels_train = final_pearson[train_index], final_timedelay[train_index], final_its[train_index], labels[train_index]
        final_pearson_val_test, final_timedelay_val_test, final_its_val_test, labels_val_test = final_pearson[test_valid_index], final_timedelay[test_valid_index], final_its[test_valid_index], labels[test_valid_index]
        stratified = stratified[test_valid_index]

    split2 = StratifiedShuffleSplit(
        n_splits=1, test_size=val_length)
    for test_index, valid_index in split2.split(final_pearson_val_test, stratified):
        final_pearson_test, final_timedelay_test, final_its_test, labels_test = final_pearson_val_test[test_index], final_timedelay_val_test[test_index], final_its_val_test[test_index], labels_val_test[test_index]
        final_pearson_val, final_timedelay_val, final_its_val, labels_val = final_pearson_val_test[valid_index], final_timedelay_val_test[valid_index], final_its_val_test[valid_index], labels_val_test[valid_index]

    train_dataset = utils.TensorDataset(
        final_pearson_train, final_timedelay_train, final_its_train, labels_train
    )

    val_dataset = utils.TensorDataset(
        final_pearson_val, final_timedelay_val, final_its_val, labels_val
    )

    test_dataset = utils.TensorDataset(
        final_pearson_test, final_timedelay_test, final_its_test, labels_test
    )
